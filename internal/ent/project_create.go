...
func (pc *ProjectCreate) AddProjectSongIDs(ids ...int) *ProjectCreate {
	pc.mutation.AddProjectSongIDs(ids...)
	return pc
}

// AddProjectSongs adds the "project_songs" edges to the ProjectSong entity.
func (pc *ProjectCreate) AddProjectSongs(p ...*ProjectSong) *ProjectCreate {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
	return pc.AddProjectSongIDs(ids...)
}
