...
func (pq *ProjectQuery) QueryProjectSongs() *ProjectSongQuery {
	query := (&ProjectSongClient{config: pq.config}).Query()
...
func (pq *ProjectQuery) WithProjectSongs(opts ...func(*ProjectSongQuery)) *ProjectQuery {
	query := (&ProjectSongClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withProjectSongs = query
	return pq
}
...
func (pq *ProjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Project, error) {
	...
	if query := pq.withProjectSongs; query != nil {
		if err := pq.loadProjectSongs(ctx, query, nodes,
			func(n *Project) { n.Edges.ProjectSongs = []*ProjectSong{} },
			func(n *Project, e *ProjectSong) { n.Edges.ProjectSongs = append(n.Edges.ProjectSongs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *ProjectQuery) loadProjectSongs(ctx context.Context, query *ProjectSongQuery, nodes []*Project, init func(*Project), assign func(*Project, *ProjectSong)) error {
