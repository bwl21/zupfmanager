...
// The ProjectSongFunc type is an adapter to allow the use of ordinary
// function as ProjectSong mutator.
type ProjectSongFunc func(context.Context, *ent.ProjectSongMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ProjectSongFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.ProjectSongMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ProjectSongMutation", m)
}
...
func (f SongFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.SongMutation); ok {
		return f(ctx, mv)
	}
